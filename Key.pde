public class Key {
  ...
  public void draw() {
    ...
    fill(this.col);
    if (this.isSpace) {
      rect(this.cornerX, this.cornerY, this.dimension*6, this.dimension);
    }
      ...
      rect(this.cornerX, this.cornerY, this.dimension*2, this.dimension);
    }
    ...
    if (!this.longKey) {
      if (this.label=="spacebar") {
        text(this.label, this.cornerX+this.dimension*3, this.cornerY+this.dimension/2);
      }
      else {
      ...
      text(this.label, this.cornerX+this.dimension, this.cornerY+this.dimension/2 );
    }
  }
  ...
  public boolean checkOn(float myX, float myY) {
    if (this.isSpace) {
      if (myX>this.cornerX && myX<this.cornerX+this.dimension*6
        && myY>this.cornerY && myY<this.cornerY+this.dimension) {
        return true;
      }
      else {
        return false;
    ...
    if (!this.longKey) {
      ...
      if (myX>this.cornerX && myX<this.cornerX+this.dimension*2
        && myY>this.cornerY && myY<this.cornerY+this.dimension*2) {
        return true;
      }
      else {
        return false;
